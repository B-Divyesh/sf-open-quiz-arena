# Open Quiz Arena: account-free live classroom quiz with no player cap (`open-quiz-arena`) — factory product contract

This repository is one product built by the Param Factory. It ships at
`https://open-quiz-arena.sociobot.in`. Artifact class: `web-with-backend`.
