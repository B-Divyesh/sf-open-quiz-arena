...
Demo state uses only `localStorage` key `demo:open-quiz-arena:step`. It never calls the room API and never reads or writes real-room session storage. **Reset demo** removes every `demo:open-quiz-arena:*` key and returns to the sample lobby. **Leave demo and create a quiz** removes those keys and opens `/create`. Leaving either demo URL also removes demo progress.
